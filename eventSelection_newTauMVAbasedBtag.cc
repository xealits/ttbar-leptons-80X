...
*/
		...
		*/
		...
		for (unsigned int count_ided_jets = 0, ijet = 0; ijet < jets.size(); ++ijet)
			...
			bool passKino = pt > 30. && fabs(eta) < 2.4; // 80x b-recommendations are for |eta|<2.4
		...
		for (size_t ijet = 0; ijet < selJetsNoLep.size(); ++ijet)
			...
			//bool hasCSVtag(jet.bDiscriminator("pfCombinedInclusiveSecondaryVertexV2BJetTags") > 0.935); // tight WP 80x
			bool hasCSVtag(jet.bDiscriminator("pfCombinedMVAV2BJetTags") > 0.185); // medium WP 80x

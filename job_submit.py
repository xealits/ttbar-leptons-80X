...
for dset_group in dsets['proc']:
    ...
    for dset in dset_group['data']:
        dset = dset['dset']
        dtag = dset['dtag']
        lumiMask = dset.get('lumiMask', '')
        ...
        if status != 0 or len(out_rows) < 4:
            print("Failed fetch files of {} dataset".format(dset))
        ...
        for i, job_chunk in enumerate(chunks(dset_files, n_files_per_job)):
            ...
            job_cfg = cfg_templ.format(input = job_chunk, lumiCert = lumiMask, dtag = dtag, outdir = outdirname, jobID = i, isMC = not isdata, outfile = outdirname + '_' + str(i) + '.root')
        ...
        with open(outdirname + '/FARM/inputs/' + dtag + '_' + 'bsub.sh') as bsub_file:
            bsub_file.write('\n'.join(dset_bsubs))

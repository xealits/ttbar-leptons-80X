...
if (projection1 != TString("x") && projection1 != TString("y") && projection1 != TString("z"))
	{
	printf("UNKNOWN projection1 GIVEN\n");
	printf("supported: x, y, z\n");
	std::cout << "Usage : " << argv[0] << usage_string << std::endl;
	return 2;
	}

if (projection2 != TString("x") && projection2 != TString("y") && projection2 != TString("z"))
	{
	printf("UNKNOWN projection2 GIVEN\n");
	printf("supported: x, y, z\n");
	std::cout << "Usage : " << argv[0] << usage_string << std::endl;
	return 2;
	}
	...
	*/
	...
	if (dtag.Contains("Data"))
		...
		for (Int_t i=0; i<=histo->GetSize(); i++)
			...
			double height  = histo->GetYaxis()->GetBinUpEdge(i) - histo->GetYaxis()->GetBinLowEdge(i);
			histo->SetBinContent(i, content/(width*height));
			}
		...
		for (int origin_n=0; origin_n<mc_jet_origins.size(); origin_n++)
			...
			*/
			...
			if (mc_jet_origin_ths[origin_n])
				...
				mc_jet_origin_ths[origin_n] = (TH2D*) histo->Clone();
			}
		//}

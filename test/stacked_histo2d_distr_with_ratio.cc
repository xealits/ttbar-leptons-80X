...
if (argc < 14)
	{
	std::cout << "Usage : " << argv[0] << " normalize logy lumi proj distr mc_distr distr_name yname rebin_factor xmin xmax ratio_range dir dtags" << std::endl;
	exit (0);
	}
...
TString proj(argv[4]);
TString distr_data(argv[5]);
TString distr_mc(argv[6]);
if (distr_mc == TString("f") || distr_mc == TString("same"))
	distr_mc = distr_data;
TString distr_name(argv[7]);
TString yname(argv[8]);

Int_t rebin_factor(atoi(argv[9]));

double xmin = atof(argv[10]);
double xmax = atof(argv[11]);
double ratio_range = atof(argv[12]);
bool xlims_set = true;
if (xmin < 0 || xmax < 0)
	xlims_set = false;

TString dir(argv[13]);
TString dtag1(argv[14]);
...
for (int i = DTAG_ARGS_START; i<argc; i++)
	...
	TH1D* histo;
	if (proj==TString("x"))
		histo = (TH1D*) ((TH2D*) files.back()->Get(distr))->ProjectionX();
	else
		histo = (TH1D*) ((TH2D*) files.back()->Get(distr))->ProjectionY();
...
cst->SaveAs( dir + "/jobsums/" + distr_mc + "_" + proj + (set_logy? "_logy" : "") + (normalize_to_data? "_normalizedToData.png" : ".png") );

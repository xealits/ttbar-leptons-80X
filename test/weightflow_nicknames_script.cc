...
const char usage_string[128] = "[--verbose] [--weight] full_selection lumi distr  dir dtags";
if (argc < 7)
	{
...
TString full_selection(argv[input_starts + 1]);
//bool full_sel = full_selection == TString("T");
double lumi = atof(argv[input_starts + 2]);
TString distro_name(argv[input_starts + 3]);

TString dir(argv[input_starts + 4]);
...
int starting_bin = ( full_selection == TString("T") ? 0 : 10);

cout << "nick,ratio" ;
for (int i=starting_bin; i<our_selection_names.size(); i++)
	cout << "," << our_selection_names[i];
cout << endl;

map<TString, TH1D*> nicked_MC;
...
for (int i = input_starts + INPUT_DTAGS_START; i<argc; i++)
	...
	if (dtag.Contains("Data"))
		...
		*/
		...
		std::pair<TString, Color_t> nick_color = dtag_nick_colour(dtag);
		TString nick = nick_color.first;
		...
		cout << endl;
		*/
		if (nicked_MC.find(nick) != nicked_MC.end())
			nicked_MC[nick]->Add(histo);
		else
			nicked_MC[nick] = histo;
...
for (map<TString, TH1D*>::iterator it=nicked_MC.begin(); it != nicked_MC.end(); it++)
	{

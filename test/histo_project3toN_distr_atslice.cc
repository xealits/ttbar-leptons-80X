...
if (argc < 5)
	...
	std::cout << "Usage : " << argv[0] << " x|y|z proj_name content_name distr1 rebin_factor x_axis_min_range x_axis_max_range x|y|z_slice slice_min slice_max filename" << std::endl;
	exit (0);
	}
...
char slice = argv[8][0];
double slice_axis_min_range = atof(argv[ 9]);
double slice_axis_max_range = atof(argv[10]);
TString filename(argv[11]);
...
cout << "slice " << slice << " axis" << endl;
cout << "from "  << slice_axis_min_range << " to " << slice_axis_max_range << endl;
cout << filename   << endl;
...
TFile f( filename.ReplaceAll(".root","") + "_" + distr1 + "_" + proj + "_atslice_" + slice + ".root", "create" );
h3->Write();
f.Write();
f.Close();
...
cst->SaveAs( filename.ReplaceAll(".root","") + "_" + distr1 + "_" + proj + "_atslice_" + slice + ".png" );

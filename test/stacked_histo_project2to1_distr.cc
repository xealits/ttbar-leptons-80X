...
for (int i = input_starts + INPUT_DTAGS_START; i<argc; i++)
	...
	if (projection == TString("x"))
		histos.push_back((TH1D*) ((TH2D*) file->Get(distr))->ProjectionX(projection));
	else if (projection == TString("y"))
		histos.push_back((TH1D*) ((TH2D*) file->Get(distr))->ProjectionY(projection));
	else
		{
		cerr << "given WRONG PROJECTION" << endl;
		return 2;
		}
